...
function M.make_openai_spec_curl_args(opts, prompt, system_prompt, is_o1)
  local url = opts.url
  local api_key = opts.api_key_name and get_api_key(opts.api_key_name)
  local data = {}
  if is_o1 then
    local modified_prompt = 'you are a helpful assistant, here is an input from a user: ' .. prompt
    data = {
...
function M.make_openai_o3_high_spec_curl_args(opts, prompt, system_prompt, is_o1)
  local url = opts.url
  local api_key = opts.api_key_name and get_api_key(opts.api_key_name)
  local data = {}
  if is_o1 then
...
function M.invoke_llm_and_stream_into_editor(opts, make_curl_args_fn, handle_data_fn, is_o1)
  ...
  local args = make_curl_args_fn(opts, prompt, system_prompt, is_o1)
...
--     replace = false,
--   }, M.make_openai_spec_curl_args, M.handle_openai_spec_data, true)
--   -- function(line)
--   --   print 'HAPPY'
--   -- end
-- end
...
-- end
-- vim.keymap.set({ 'n', 'v' }, '<leader>j', anthropic_help, { desc = 'llm test help' })
return M
